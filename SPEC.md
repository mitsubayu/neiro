...
| F27 | 日本語対応 | en/ja の Localizable.strings。コード側の非 SwiftUI 文字列は `String(localized:)` |
...
- 健全性: **無検出の曲開始が2回続き**、かつ直近60秒に検出がなければストリームが死んだと判断し、
  検出プロセスを再起動(復元も走るので現在の曲のレートも即座に戻る)。連発防止に60秒のクールダウン。
  ギャップレス再生など「同一フォーマットの継続」ではログが出ないため、1回の無検出では判断しない
...
**横3列・1206×378 の固定サイズ**(縦に積むと 760pt を超え、折りたたみのたびにウィンドウが
伸縮して不具合の温床だった)。
