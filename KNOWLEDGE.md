...
- **Music のログには「そのトラックのレート」ではない行が混ざる**。`Output format:` を
  出すのは実デコーダ(`ACAppleLosslessDecoder` / `ACMP4AACBaseDecoder`)だけでなく、
  総称ラッパの `ACCPEDecoderWrapper` もで、後者は**出力デバイス側のレート**も報告する。
  8分間の実測で 48000 Hz を出したのはラッパ行だけだった。これを拾うとデバウンスの最後に
  居座って「レート差なし=切り替え不要」と判断され、**常に 48 kHz のまま**になる。
  → レートはデコーダ名が入っている行だけから採る
- **AppleScript の問い合わせ失敗を「状態が変わった」と解釈しない**。切り替え直後の Music は
  読み込みで忙しく `name of current track` が普通にタイムアウトする。これを
  「ユーザーが別の曲にした」と見なして頭出しの巻き戻しを取りやめていたため、
  ミュートした曲頭がそのまま消えていた。nil は「不明」であって「別の曲」ではない
- **「レート一致 → idle」は切り替え成功の直後にも成立する**。エンジンは新レートで
  組み上がっているが、resume(seek + play)はまだ走っている最中。ここで idle 側の
  後始末を走らせると seek が二重になり**曲頭が2回鳴る**。in-flight 中は手を出さない
- 同一アルバム内はレートが同じ。切り替えのテストは**アルバムをまたいで**行う
  (`docs/debugging-rates.md` に実測レート表)
...
### 3.4.5 【アンチパターン】SwiftUI の内容に合わせてウィンドウをリサイズする
- **症状**: 折りたたみセクションを開いて閉じるとパネル上部に余白が残る。
  直そうとして**クラッシュ**(`Thread stack size exceeded due to excessive recursion`)
- **真因**: borderless パネル + `NSHostingController.sizingOptions = [.preferredContentSize]` は、
  SwiftUI のジオメトリ更新 → ウィンドウのリサイズ → Auto Layout → 再びジオメトリ更新…と
  再帰する。リサイズ通知を受けて自分でウィンドウを動かすと**確実に**落ちる
  (`DispatchQueue.main.async` で遅延させても2回目の開閉で落ちた)
- **対策**: **開くときに一度だけサイズを決め、開いている間は一切変えない**。
  中身は `ScrollView` に入れておけば、セクションが伸びてもウィンドウは動かない。
  「ウィンドウが動かない」=余白も再帰も原理的に発生しない
- **検証法**: アクセシビリティでウィンドウの `position`/`size` を毎回読む。
  4回開閉して `top=32 h=760` が不変であることを機械的に確認できた
...
### 3.7 【アンチパターン】一部だけローカライズするのに既定テーブルを使う
- **症状**: 英語のままにしたいパネルの「Presets」が、ヘルプ用に入れた訳を拾って日本語になる
- **真因**: SwiftUI の `Text("Presets")` は**文字列そのものがキー**。ヘルプの見出しに同じ語を
  使っていたため、同じ `Localizable.strings` にある訳が適用された
- **対策**: 部分ローカライズは**専用テーブル**に分ける(`Help.strings` +
  `String(localized:table:)`)。既定テーブルを空にしておけば、訳したくない文字列は
  構造的に訳されない
